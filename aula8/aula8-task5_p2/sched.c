...
volatile tcb_ll_t* find_next_head(){
  ...
  while (i != head) {
    // Se a tarefa estiver dormindo e já tiver passado o tempo de dormir...
	if (i->state == TASK_SLEEPING && ticks >= i->sleep_until) {
      // ... ela acorda e volta para o estado "pronta"
      i->state = TASK_READY;
    }
	  
    if (i->state != TASK_TERMINATED && i->state != TASK_SLEEPING) {
      return i; // Retorna o primeiro processo que não foi finalizado e não esta dormindo
    }
    i = i->next;
  }

  return NULL; // Se todos os processos foram finalizados ou estão dormindo, retorna NULL
}
